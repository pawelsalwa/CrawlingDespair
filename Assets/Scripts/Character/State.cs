using System;
using Character.Setup;
using UnityEngine;

namespace Character
{
	public abstract class State
	{
		public string TypeName => GetType().Name;
		
		protected readonly Character Character;
		protected readonly StateSetup setup;
		protected readonly Fsm fsm;
		protected readonly PlayerInput PlayerInput;

		protected float stateProgress = 0f;
		protected float fixedStateProgress = 0f;

		protected State(Fsm fsm, StateSetup setup)
		{
			this.setup = setup;
			this.fsm = fsm;
			Character = fsm.Character;
			PlayerInput = Character.playerInput;
		}

		protected virtual State DefaultNextState => fsm.DefaultState;
		protected virtual float StateDuration => setup.ExitTime;
		protected float ProgressPercentage => stateProgress / setup.ExitTime;

		protected abstract Action AnimCall { get; }

		protected bool Active { get; private set; } = false;

		public void Enter()
		{
			stateProgress = fixedStateProgress = 0f;
			Active = true;
			if (AnimCall == null) Debug.Log($"<color=white> {Character.gameObject.name}: no anim set for state: {GetType()}</color>", Character.gameObject);
			else AnimCall.Invoke();
			OnEnter();
		}

		public void Update()
		{
			OnUpdate();
			
			if ((stateProgress += Time.deltaTime) >= StateDuration && setup.HasExitTime)
			{
				OnBeforeTransitionByExitTime();
				RequestTransition(DefaultNextState);
			}
		}

		public void FixedUpdate()
		{
			fixedStateProgress += Time.fixedDeltaTime;
			OnFixedUpdate();
		}

		public void Exit()
		{
			OnExit();
			Active = false;
		}
		
		protected virtual void OnEnter() { }
		protected virtual void OnUpdate() { }
		protected virtual void OnFixedUpdate() { }
		
		/// <summary>
		/// This is called before state is transitioning to next state due to exit time.
		/// Useful if we want to apply logic when state duration comes to an end and state is not exited by any other factor.
		/// Can be used to override <see cref="DefaultNextState"/>.
		/// </summary>
		protected virtual void OnBeforeTransitionByExitTime() { }
		protected virtual void OnExit() { }

		protected void RequestTransition(State targetState) => fsm.PerformTransition(targetState);
	}
}